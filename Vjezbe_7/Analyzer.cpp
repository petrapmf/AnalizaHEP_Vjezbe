...
void Analyzer::Loop()
{
   ...
   void Analyzer::Plot(TString path) 
   {
       ...
       canvas->SaveAs("Lepton_" + path + ".pdf");
       ...
       std::cout << histohiggs->Integral() << std::endl;
       canv->SaveAs("Higgs_" + path + ".pdf");

       auto cnvs = new TCanvas("cnvs", "cnvs", 1300, 800);
       ...
       hs->SetTitle("Reconstructed 4lepton Mass");
       hs->GetXaxis()->SetTitle("m_{4l} [GeV]");
       hs->GetYaxis()->SetTitle("Events / 2 GeV");

       TLegend* leg = new TLegend(.7, .75, .89, .89);
       leg->AddEntry(h_signal, "ggH125", "f");
       leg->AddEntry(h_background, "qqZZ", "f");
       leg->Draw();

       cnvs->SaveAs("Combination.pdf");*/
   }
